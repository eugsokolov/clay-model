...
AUTHORIZED_USERS = {"eugene", "szymon", "yc-collab",}
...
class ClayModelAPI(ls.LitAPI):
    ...
    def predict(self, request):
        if request["whoami"] not in AUTHORIZED_USERS:
            return JSONResponse(status_code=401, content={"reason": "Please reach out to host to authorize access"})

        lat, lon = request["lat"], request["lon"]
        start, end = request["startdate"], request["enddate"]
        size = request.get("size", 256)
        embeddings, stack = get_embeddings(lat, lon, start, end, size=size, model=self.model)
